<p align="center">
  <img src="PASTE_YOUR_IMAGE_URL_HERE" width="100%" alt="Alan Jibin Banner">
</p>

# Hi there, I'm Alan! 👋

I'm a **B.Tech student** at St. Joseph's College of Engineering and Technology, Palai. I build at the intersection of hardware and software, with a focus on **Robotics**, **Computer Vision**, and **Embedded Systems**.

---

### 🛠 Tech Stack & Tools
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)
![Arduino](https://img.shields.io/badge/-Arduino-00979D?style=for-the-badge&logo=Arduino&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-%23EE4C2C.svg?style=for-the-badge&logo=PyTorch&logoColor=white)

- **AI/ML:** YOLOv8, OpenCV, TensorFlow
- **Hardware:** ESP32, NodeMCU, Sensors (IMU, Flex, Gas)
- **Interests:** Cloud Services & Information Security

---

### 🚀 Featured Engineering Projects

#### 🔍 Automated Metal Chain Crack Detection System
* **The Problem:** Manual inspection of industrial chains is slow and prone to human error.
* **The Solution:** Developed a real-time detection rig using **YOLOv8** to identify surface deformations and cracks.
* **Tech:** Python, OpenCV, PyTorch, and a custom hardware rig.

#### 🧤 Assistive Haptic Glove
* **The Problem:** Limited communication tools for individuals with speech or motor impairments.
* **The Solution:** A wearable glove that translates hand gestures into digital commands or speech.
* **Tech:** ESP32, Flex sensors, and IMUs for motion tracking.

#### 🤖 Road Safety Line Repainting Robot
* **The Tech:** Autonomous navigation using line-following algorithms and PID control for precision marking.

---

### 📫 Connect with me
[LinkedIn](YOUR_LINK_HERE) | [Portfolio](YOUR_LINK_HERE)

> 🐕 **Personal Assistant:** Milo the Beagle (Head of Morale & Barking)
